...
A distro made for the CENTRO MEDICO OSCAR ROMERO (COR) from openmrs-distro-pihemr(https://github.com/PIH/openmrs-distro-pihemr)
...
```
$ mvn openmrs-sdk:setup -DserverId=[serverId] -Ddistro=org.openmrs.distro:cor:1.0.0-SNAPSHOT
```

* When prompted, select the "pih.config" value to use.  This determines the site-specific configuration to be applied to your server environment. Since this is a distro only used for COR, we'll use this:
...
$ mvn openmrs-sdk:setup \
    -DserverId=[serverId] \
    -Ddistro=org.openmrs.distro:cor:1.0.0-SNAPSHOT \
...
For this distro we'll use this configuration:
...
For instance, for the COR configuration the command is:

```
git clone https://github.com/PIH/openmrs-config-pihemr.git
git clone https://github.com/PIH/openmrs-config-ses.git
```
...
|Distro-sescor|https://github.com/PIH/openmrs-distro-sescor|
|Module-pihcore|https://github.com/PIH/openmrs-module-pihcore|

You can watch them using the following command:
```
mvn openmrs-sdk:watch
```
Once everything is ready we run this:
...
### Step 6: Create an account that is a provider
